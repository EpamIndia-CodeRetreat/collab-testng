...
public class HelperReadJsonAndCreateClass {
    ...
    private static final String POM_CONFIG_FILE_PATH = FilenameUtils.normalize("src"+FILE_SEPARATOR
            +"main"+FILE_SEPARATOR
    ...
    // Logger
    private static final Logger logger = Logger.getLogger("HelperReadJsonAndCreateClass");
    ...
    public void readJSON() {
        ...
        String projectDirectory = FilenameUtils.normalize(System.getProperty("user.dir"));
        filePath = FilenameUtils.normalize(projectDirectory + prop.getProperty("inputFilePath"));
        pageObjectRecordPath = FilenameUtils.normalize(projectDirectory + prop.getProperty("objectRecordPath"));
        pageObjectsPath = FilenameUtils.normalize(projectDirectory + prop.getProperty(PAGE_OBJECTS_PATH));
        verificationPath = FilenameUtils.normalize(pageObjectsPath + prop.getProperty("verificationPath"));
        packageName = prop.getProperty("packageName");
        actionsPath = FilenameUtils.normalize(pageObjectsPath + prop.getProperty(ACTIONS_PATH));
        try {
    ...
    private void readPageObjectsFromJson() {
        ...
        if (Files.exists(Paths.get(filePath))) {
            ...
            if (files != null && files.length > 0) {
                ...
                logger.info("No JSON File Found At Given Location");
            }
        } else {
            logger.info("File Path To JSON Files Is Invalid");
        }
    }
    ...
    private void validateOperationIfPageExists(String currentPageObjectOperation, PageObject pageObject){
        try{
            if(currentPageObjectOperation.equalsIgnoreCase(PageObjectOperationConstants.OPERATION_CREATE)){
                logger.info("PageObject "+pageObject.getPageName()+" already exists!");
    ...
    private void modifyPageObject(PageObject pageObject) {
        try{
            if (pageObjectMap.containsKey(pageObject.getPageName())) {
                logger.info("Duplicate Entry For PageObject Class "+pageObject.getPageName());
            } else {
    ...
    public void cleanRecordDirectory(){
        try {
            FileUtils.deleteDirectory(new File(FilenameUtils.normalize(System.getProperty("user.dir")+File.separator+prop.getProperty("objectRecordPath"))));
